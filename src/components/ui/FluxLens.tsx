...
export function FluxLens({
  ...
  const handlePointerEnter = (event: React.PointerEvent<HTMLElement>) => {
    ...
    const root = rootRef.current;
    if (root) {
      root.style.setProperty("--flux-lens-x", `${currentRef.current.x}%`);
      root.style.setProperty("--flux-lens-y", `${currentRef.current.y}%`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <span className="flux-lens__overlay" aria-hidden="true">
        <span className="flux-lens__overlay-content">{children}</span>
      </span>
    </Component>
  );
}
