...
export function EditorCanvas({ doc, layout, selectedCellId, onSelectCell, materials }: EditorCanvasProps) {
  return (
            ...
            <div className="flex-1 min-h-0 space-y-6 overflow-auto">
              {layout.grids.map((grid) => (
                <section key={grid.name} className="space-y-2">
                  ...
                  >
                    {grid.cells.map((cell) => {
                      const material = materials.find((m) => m.name === cell.materialRef);
                      const materialColor = material?.color ?? cell.materialColor;
                      const materialLabel = material?.label ?? material?.name ?? cell.materialLabel;
                        ...
                        >
                          {cell.materialRef && (
                            <div
                              ...
                              style={{ backgroundColor: materialColor ?? "#00cdfe" }}
                            />
                          )}
                          ...
                          {materialLabel && (
                            <div
                              ...
                              style={
                                materialColor
                                  ? { backgroundColor: `${materialColor}20`, borderColor: materialColor }
                                  : undefined
                              }
                            >
                              <span
                                className="h-2 w-2 rounded-full border border-slate-400"
                                style={materialColor ? { backgroundColor: materialColor } : undefined}
                              />
